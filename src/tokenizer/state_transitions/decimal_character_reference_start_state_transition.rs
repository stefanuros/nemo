...
pub fn decimal_character_reference_start_state_transition(
  c: Option<char>, 
  ...
  println!("Decimal Character Reference Start State c: '{:?}'", c);

  return match c {
    Some(x) if x.is_ascii_digit() => decimal_character_reference_start_state_transition_ascii_digit(c, current_state),
    _ => decimal_character_reference_start_state_transition_anything_else(c, current_state, return_state, current_token, temporary_buffer)
  }
}

fn decimal_character_reference_start_state_transition_ascii_digit(
  c: Option<char>,
  current_state: &mut DataState
) -> (Option<Vec<Token>>, bool) {
  println!("Decimal Character Reference Start State ASCII Hexdigit: '{:?}'", c);

  *current_state = DataState::DecimalCharacterReferenceState;

  return (None, true);
}

fn decimal_character_reference_start_state_transition_anything_else(
  c: Option<char>,
  current_state: &mut DataState,
  return_state: &mut DataState,
  current_token: &mut Option<Token>,
  ...
  println!("Decimal Character Reference Start State Anything Else: '{:?}'", c);

  absence_of_digits_in_numeric_character_reference_parse_error::error(DataState::DecimalCharacterReferenceStartState.to_string(), c);
...
mod tests {
  ...
  #[test]
  fn test_decimal_character_reference_start_state_transition_ascii_digit() {
    const C: Option<char> = Some('5');
    let mut current_state: DataState = DataState::DecimalCharacterReferenceStartState;
    ...
    let result = decimal_character_reference_start_state_transition(
      C, 
      &mut current_state,
      &mut return_state,
      &mut current_token,
  ...
  #[test]
  fn test_decimal_character_reference_start_state_transition_anything_else() {
